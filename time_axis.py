...
def _time_init(ctx):
    ...
    if 'units' in data.variables['time'].ncattrs():
        ...
        raise _Exception('Time units attribute is missing.')
    ...
    if 'calendar' in data.variables['time'].ncattrs():
        ...
        raise _Exception('Time calendar attribute is missing.')
...
def _time_axis_processing(inputs):
    ...
    if not _last_date_checker(_date_print(last_hp), _date_print(end_date)) and not _last_date_checker(_date_print(last_lp), _date_print(end_date)):
        ...
        _log('warning', 'ERROO3 - Inconsistent last and end dates for {0}'.format(filename))
    else:
        if _last_date_checker(_date_print(last_hp), _date_print(end_date)):
            status.last = _date_print(last_hp)
            status.axis = axis_hp
        elif _last_date_checker(_date_print(last_lp), _date_print(end_date)):
            status.last = _date_print(last_lp)
            status.axis = axis_lp
        ...
        if instant and ('time_bnds' in data.variables.keys()):
            ...
            _log('warning', 'ERROO4 - Inconsistent time_bnds with instant time for {0}'.format(filename))
